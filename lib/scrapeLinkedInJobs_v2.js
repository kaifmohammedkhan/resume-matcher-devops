...
// ⚙️ Allow longer Vercel execution (Pro plan supports up to 60s)
export const config = {
  maxDuration: 60
};
...
export async function scrapeLinkedInJobs(input = [], location = '', workMode = 'All') {
  ...
  const searchUrl = `LINKEDIN_URL?keywords=${encodeURIComponent(
    sanitizedQuery
  ...
  try {
    ...
    // 🕒 Retry helper (handles LinkedIn’s slow load)
    const waitForJobs = async (selector, retries = 3) => {
      for (let i = 0; i < retries; i++) {
        try {
          console.log(`⏳ Waiting for job listings... (Attempt ${i + 1})`);
          await page.waitForSelector(selector, { timeout: 20000 });
          return true;
        } catch (e) {
          console.warn(`⚠️ Jobs not loaded yet, retrying... (${i + 1}/3)`);
          await new Promise(r => setTimeout(r, 4000));
        }
      }
      throw new Error('Failed to load LinkedIn job listings after retries.');
    };

    await waitForJobs('.jobs-search__results-list li, .base-card');
    await new Promise(r => setTimeout(r, 3000)); // give DOM extra time
