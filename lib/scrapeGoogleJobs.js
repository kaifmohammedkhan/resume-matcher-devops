...
// Google-based LinkedIn Job Scraper (Throttled + Resilient)
export async function scrapeGoogleJobs(input = [], location = "", workMode = "All") {
  ...
  const allResults = [];
  const maxResults = 50; // 🔒 Reduce to avoid quota exhaustion
  const pageSize = 10;
  ...
  for (let start = 1; start <= maxResults; start += pageSize) {
    ...
    try {
      ...
      if (status === 429) {
        console.warn("⚠️ Rate limit hit — stopping scrape early.");
        break;
      }
    }
  }
