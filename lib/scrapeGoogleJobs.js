...
export async function scrapeGoogleJobs(input = [], locationInput = "", workModeInput = "All") {
  ...
  // 🧪 WireMock Mode Check for Load Testing
  if (process.env.WIREMOCK_URL) {
    try {
      const url = `${process.env.WIREMOCK_URL}/search`;
      const { data } = await axios.get(url);
      ...
      console.error("WireMock call failed:", err.message);
      return [];
    }
  }

  // 🧪 Legacy Mock Mode (optional fallback)
    ...
    const mockJobs = [
      {
        ...
        job_description: "This is a comprehensive mock job description long enough to satisfy length validation rules for testing pipelines safely without hitting real API limits.",
        work_mode: "Remote",
        job_country: "Global",
        source: "Mock",
      },
      {
        ...
        job_description: "Another mock job description used to simulate successful database inserts, scoring overhead, and rendering workflows during high volume k6 runs.",
        work_mode: "Onsite",
        job_country: "Global",
        source: "Mock",
      }
