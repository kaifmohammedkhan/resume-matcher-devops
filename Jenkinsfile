...
NODE
        ...
        stage('Security Enhancements') {
            ...
            stages {
                ...
                stage('Sign Docker Images (GHCR & Docker Hub)') {
                    steps {
                        withCredentials([
                            string(credentialsId: 'COSIGN_PASSPHRASE', variable: 'COSIGN_PASSWORD')
                        ]) {
                            sh '''
                                ...
                                ghcr_image_short="ghcr.io/$GITHUB```groovy
pipeline {
    agent any
                ...
                stage('Extract metadata (tags, labels) for Docker') {
                    steps {
                        sh '''
                            ...
                            export OCI_CREATED="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
                            export OCI_REVISION="$GITHUB_SHA"
                            export OCI_SOURCE="[https://github.com/$GITHUB_REPOSITORY](https://github.com/$GITHUB_REPOSITORY)"

                            GITHUB_SHORT_SHA="$(printf '%.7s' "$GITHUB_SHA")"

                            export OCI_VERSION="sha-$GITHUB_SHORT_SHA"

                            cat > reports/docker/docker-labels.env <<EOF !="unknown" " "");')" "$DOCKERHUB_USERNAME/resume-matcher-devops:latest" "$DOCKERHUB_USERNAME/resume-matcher-devops:sha-$GITHUB_SHORT_SHA" "$IMAGE_DIGEST" "ERROR: "IMAGE_DIGEST="$IMAGE_DIGEST" "ghcr.io/$GITHUB_REPOSITORY:latest" "ghcr.io/$GITHUB_REPOSITORY:sha-$GITHUB_SHORT_SHA" "org.opencontainers.image.created="$OCI_CREATED" "org.opencontainers.image.revision="$OCI_REVISION" "org.opencontainers.image.source="$OCI_SOURCE" "org.opencontainers.image.version="$OCI_VERSION" $GITHUB_SHA")" $IMAGE_DIGEST" ''' '{{.Manifest.Digest}}')" *) --format --label --metadata-file --platform --push -e -n -s -t -z . 1 ;; Docker EOF GITHUB_SHORT_SHA="$(printf '%.7s' " IMAGE_DIGEST="$(docker buildx imagetools inspect \
                                    " Invalid OCI_CREATED="$OCI_CREATED" OCI_REVISION="$OCI_REVISION" OCI_SOURCE="$OCI_SOURCE" OCI_VERSION="$OCI_VERSION" [ \ ]; and build buildx case const d="JSON.parse(fs.readFileSync(" digest: docker echo esac exit fi fs"); ghcr.io/$GITHUB_REPOSITORY:sha-$GITHUB_SHORT_SHA" if image image') in linux/amd64,linux/arm64 process.stdout.write(d["containerimage.digest"] push reports/docker/build-metadata.json reports/docker/build-metadata.json","utf8"));" reports/docker/docker-labels.env set sh sha256:*) stage('Build steps test then { || }> reports/docker/image-digest.env
                        '''
                ...
                stage('Record immutable image digest') {
                    steps {
                        sh '''
                            ...
                            {
                                echo "IMAGE_DIGEST=$IMAGE_DIGEST"
                                echo "IMAGE_TAGS<<EOF_TAGS" "$IMAGE_TAGS" "EOF_TAGS" '%s\\n' echo printf }> reports/docker/image-metadata.txt
                    ...
                    steps {
                        sh '''
                            ...
                            if ! command -v cosign >/dev/null 2>&1; then
                                ...
                                curl -sSfL \
                                    "[https://github.com/sigstore/cosign/releases/download/$](https://github.com/sigstore/cosign/releases/download/$){COSIGN_VERSION}/cosign-linux-amd64" \
                                    -o /tmp/cosign
                ...
                stage('Install Syft') {
                    steps {
                        sh '''
                            ...
                            curl -sSfL [https://raw.githubusercontent.com/anchore/syft/main/install.sh](https://raw.githubusercontent.com/anchore/syft/main/install.sh) \
