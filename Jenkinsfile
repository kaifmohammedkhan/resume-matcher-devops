            ...
            stages {
                ...
                stage('Initialize GitHub metadata') {
                    steps {
                        script {
                            ...
                            env.GITHUB_REF_NAME = sh(script: 'git rev-parse --abbrev-ref HEAD', returnStdout: true).trim()
                            env.GITHUB_RUN_NUMBER = env.BUILD_NUMBER
                            env.GITHUB_ACTOR = sh(script: "git log -1 --format='%an'", returnStdout: true).trim()
                            env.IMAGE_TAGS =
                ...
                stage('Set up Docker Buildx') {
                    steps {
                        sh '''
                            set -e
                            docker buildx inspect resume-matcher-builder >/dev/null 2>&1 ||                               docker buildx create --name resume-matcher-builder --driver docker-container --use
...
EOF
                ...
                stage('Generate Docker Build & Push HTML Report') {
                    steps { sh '''node --input-type=commonjs <<'NODE'
...
NODE''' }
                ...
                stage('Upload Docker Build & Push Report') {
                    steps {
                        archiveArtifacts artifacts: 'reports/docker/docker-build-push-report.html,reports/docker/image-metadata.txt', fingerprint: true
                    }
                }

                stage('Install Nodemailer') {
                    steps { sh 'npm install nodemailer@9.0.3' }
                }
            ...
            stages {
                ...
                stage('Download Stage 1 Build Report') {
                    steps {
                        ...
                        sh '''
                            ...
                            cp reports/docker/docker-build-push-report.html reports/docker-build/docker-build-push-report.html
                            cp reports/docker/image-metadata.txt reports/docker-build/image-metadata.txt
                        '''
                    }
                }
                ...
                stage('Load immutable image digest') {
                    steps {
                        script {
                            env.IMAGE_DIGEST = sh(
                                script: "set -e; test -s reports/docker-build/image-metadata.txt; sed -n 's/^IMAGE_DIGEST=//p' reports/docker-build/image-metadata.txt | head -n 1",
                                returnStdout: true
                            ).trim()
                        }
                        sh '''
                            ...
                            case "$IMAGE_DIGEST" in
                              sha256:*) ;;
                              *) echo "ERROR: Invalid image digest: $IMAGE_DIGEST"; exit 1 ;;
                            esac
                            echo "=============================================="
                ...
                stage('Verify Cosign installation') { steps { sh 'cosign version' } }
                ...
                stage('Verify Syft installation') { steps { sh 'syft version' } }
                ...
                stage('Verify GHCR image exists') {
                    steps {
                        sh 'docker manifest inspect ghcr.io/$GITHUB_REPOSITORY@$IMAGE_DIGEST'
                    }
                }

                stage('Verify Docker Hub image exists') {
                    steps {
                        sh 'docker manifest inspect $DOCKERHUB_USERNAME/resume-matcher-devops@$IMAGE_DIGEST'
                    }
                }
                ...
                stage('Cosign Sign GHCR Image') {
                    steps {
                        withEnv(['COSIGN_YES=true']) {
                            sh 'cosign sign ghcr.io/$GITHUB_REPOSITORY@$IMAGE_DIGEST'
                        }
                    }
                }
                ...
                stage('Cosign Sign Docker Hub Image') {
                    steps {
                        withEnv(['COSIGN_YES=true']) {
                            sh 'cosign sign $DOCKERHUB_USERNAME/resume-matcher-devops@$IMAGE_DIGEST'
                        }
                    }
                }
                ...
                stage('Verify Cosign Signature — GHCR') {
                    steps {
                        sh '''
                            cosign verify                               --certificate-identity-regexp="https://github.com/kaifmohammedkhan/resume-matcher-devops/.github/workflows/.*@refs/heads/main"                               --certificate-oidc-issuer="https://token.actions.githubusercontent.com"                               --output json                               ghcr.io/$GITHUB_REPOSITORY@$IMAGE_DIGEST                               > reports/docker/cosign-ghcr-verify.json
                ...
                stage('Verify Cosign Signature — Docker Hub') {
                    steps {
                        sh '''
                            cosign verify                               --certificate-identity-regexp="https://github.com/kaifmohammedkhan/resume-matcher-devops/.github/workflows/.*@refs/heads/main"                               --certificate-oidc-issuer="https://token.actions.githubusercontent.com"                               --output json                               $DOCKERHUB_USERNAME/resume-matcher-devops@$IMAGE_DIGEST                               > reports/docker/cosign-dockerhub-verify.json
                ...
                stage('Generate GHCR SPDX SBOM') {
                    steps {
                        sh '''
                            syft ghcr.io/$GITHUB_REPOSITORY@$IMAGE_DIGEST -o spdx-json > reports/docker/sbom-ghcr.json
                ...
                stage('Generate Docker Hub SPDX SBOM') {
                    steps {
                        sh '''
                            syft $DOCKERHUB_USERNAME/resume-matcher-devops@$IMAGE_DIGEST -o spdx-json > reports/docker/sbom-dockerhub.json
                ...
                stage('Cosign Attest SPDX SBOM — GHCR') {
                    steps {
                        withEnv(['COSIGN_YES=true']) {
                            sh '''
                                cosign attest                                   --predicate reports/docker/sbom-ghcr.json                                   --type spdxjson                                   ghcr.io/$GITHUB_REPOSITORY@$IMAGE_DIGEST
                ...
                stage('Cosign Attest SPDX SBOM — Docker Hub') {
                    steps {
                        withEnv(['COSIGN_YES=true']) {
                            sh '''
                                cosign attest                                   --predicate reports/docker/sbom-dockerhub.json                                   --type spdxjson                                   $DOCKERHUB_USERNAME/resume-matcher-devops@$IMAGE_DIGEST
                ...
                stage('Verify SPDX SBOM Attestation — GHCR') {
                    steps {
                        sh '''
                            cosign verify-attestation                               --certificate-identity-regexp="https://github.com/kaifmohammedkhan/resume-matcher-devops/.github/workflows/.*@refs/heads/main"                               --certificate-oidc-issuer="https://token.actions.githubusercontent.com"                               --type spdxjson                               --output json                               ghcr.io/$GITHUB_REPOSITORY@$IMAGE_DIGEST                               > reports/docker/attestation-ghcr.json
                ...
                stage('Verify SPDX SBOM Attestation — DOCKER HUB') {
                    steps {
                        sh '''
                            cosign verify-attestation                               --certificate-identity-regexp="https://github.com/kaifmohammedkhan/resume-matcher-devops/.github/workflows/.*@refs/heads/main"                               --certificate-oidc-issuer="https://token.actions.githubusercontent.com"                               --type spdxjson                               --output json                               $DOCKERHUB_USERNAME/resume-matcher-devops@$IMAGE_DIGEST                               > reports/docker/attestation-dockerhub.json
                ...
                stage('Create raw security evidence ZIP') {
                    steps {
                        sh '''
                            set -e
                            cd reports/docker
                            zip -j docker-security-raw-evidence.zip                               cosign-ghcr-verify.json                               cosign-dockerhub-verify.json                               sbom-ghcr.json                               sbom-dockerhub.json                               attestation-ghcr.json                               attestation-dockerhub.json
                            cd ../..
